...
const productImage = document.getElementById('productImage');// Imagen del producto
let currentColor = 'gray'; // Color inicial
let currentProduct = 'XLS'; // Producto inicial
...
function preloadImages() {
  ...
  for (let i = 1; i <= totalFrames; i++) {
    const imgSrc = `/images/modelos/yaris-sedan/XLS/${currentColor}/${i}.png`;
...
function updateImage() {
  productImage.src = `/images/modelos/yaris-sedan/XL/${currentColor}/${currentFrame}.png`;
}
...
colorButtons.forEach(button => {
  button.addEventListener('click', () => {
    ...
    function openTab(tabName) {
        ...
        // Aplicar la clase 'active' al botón de la tab actua
         if (tabName === 'automatico-xls') {
      ...
      // Actualizar la información del auto
      if (manualXlsTab && automaticoXlsTab && automaticoXlsPackTab) { // Si los elementos existen en el DOM (no son null) entonces se ejecuta el código dentro de las llaves {}
            manualXlsTab.click();
